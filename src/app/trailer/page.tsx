...
export default function Trailer() {
  ...
  // ?auto → avvio automatico dopo il mount (evita il mismatch di idratazione).
  useEffect(() => {
    if (new URLSearchParams(window.location.search).has("auto")) setStarted(true);
  }, []);
  ...
  // Espone progresso/durata per lo script di rendering dell'MP4 (cattura headless).
  useEffect(() => {
    const g = window as Window & { __t?: number; __total?: number };
    g.__t = t;
    g.__total = total;
  }, [t, total]);
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-[#05070d] text-slate-100" style={{ fontFamily: "ui-sans-serif, system-ui, -apple-system, sans-serif" }}>
      ...
      {/* Barre cinematografiche (letterbox) — stile inline robusto */}
      <div className="pointer-events-none absolute inset-x-0 top-0 bg-black" style={{ height: "5.5vh" }} />
      <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-black" style={{ height: "5.5vh" }} />

      {/* Sottotitoli voce narrante (cronometrati) */}
      {sub && sottotitolo && (
        <div
          className="pointer-events-none absolute left-1/2 px-6 text-center"
          style={{ bottom: "10vh", transform: "translateX(-50%)", maxWidth: "46rem" }}
        >
          <p className="text-lg font-light leading-snug text-slate-200" style={{ textShadow: "0 2px 12px rgba(0,0,0,0.85)" }}>
            {sottotitolo}
          </p>
        </div>
      )}
