...
export const TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    name: "attiva_gesti",
    description:
      "Attiva la MODALITÀ GESTI: la mano davanti alla telecamera diventa il MOUSE del computer, su TUTTO lo schermo (qualsiasi app, sito, Finder, Dock, non solo i pannelli di ORION). Come funziona: si PUNTA con l'indice (un pallino celeste segue la punta del dito); il CLICK sinistro si fa col CENNO dell'indice (lo pieghi come un tap nell'aria — due cenni rapidi = doppio click per aprire app/file); si TRASCINA unendo pollice+indice e muovendo (sposta finestre, file nel cestino, selezioni); il TASTO DESTRO si fa unendo pollice+medio; due mani unite ridimensionano la finestra sotto. Usalo quando l'utente dice 'modalità gesti', 'voglio usare le mani', 'controllo a gesti', 'comandare col dito'. È SOLO Desktop e serve il permesso Accessibilità (lo chiede la prima volta). Diverso dalla modalità visione (che assiste le attività manuali). A voce una frase breve tipo 'Gesti attivi: punta con l'indice, cenno per cliccare'.",
    input_schema: { type: "object", properties: {} },
  },
  {
    name: "riassumi_link",
