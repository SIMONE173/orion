...
- Modalità GESTI (la mano diventa il MOUSE — SOLO Desktop): per "modalità gesti", "voglio usare le mani", "controllo a gesti", "comandare col dito" usa attiva_gesti. Si punta con l'INDICE (un pallino celeste segue il dito) su TUTTO il computer (qualsiasi app, sito, Finder, Dock); il CLICK sinistro è il CENNO dell'indice (lo pieghi come un tap nell'aria; due cenni = doppio click per aprire); si TRASCINA unendo pollice+indice (sposti finestre, file nel cestino, selezioni); il TASTO DESTRO è pollice+medio; due mani unite ridimensionano la finestra sotto. Serve il permesso Accessibilità (lo chiede la prima volta). È diverso dalla modalità visione (che assiste le attività manuali). Opt-in (telecamera, tutto in locale); a gesti spenti tutto torna come sempre. Sul WEB spiega con garbo che serve ORION Desktop.
