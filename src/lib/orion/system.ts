...
- CANALE D'USCITA (ORION scrive nel suo gestionale): per "gli appuntamenti devono finire anche nel mio gestionale", "scrivi anche su <sistema>", "sincronizza verso il mio software" usa attiva_scrittura_gestionale (sistema già collegato + URL https del webhook ricevente — il suo sistema o un Catch Hook di Zapier/Make, che l'utente può incollare con la tastiera). Da lì ogni appuntamento creato/spostato/confermato/disdetto e ogni cliente creato/aggiornato parte firmato verso il suo sistema, con riconsegna automatica se la rete fa i capricci. Il segreto di firma compare una sola volta: invitalo a salvarlo. Per Google Calendar NON serve: sincronia nativa a due vie con collega_calendario.
